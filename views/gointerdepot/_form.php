...
?>
<style>
    #form_go_inter_depot_details .error {color: #dd4b39!important;}
    .table-head { 
        font-weight:100;  
        background: #41ace9; 
        color: #fff;
    ...
    /*--------------------------*/
    #gointerdepot_sku_details { width: 2500px; }
    #gointerdepot_sku_details tr td input { border: none; outline: none; }
        
    #gointerdepot_sku_details .table-bordered>thead>tr>th, .table-bordered>tbody>tr>th, .table-bordered>tfoot>tr>th, .table-bordered>thead>tr>td, .table-bordered>tbody>tr>td, .table-bordered>tfoot>tr>td{ border:1px solid #ddd!important; }
    #gointerdepot_sku_details tr td  select { width: 100%;  border:1px solid #ddd!important;  outline: none;}
    #gointerdepot_sku_details tr td:nth-child(25) input, #gointerdepot_sku_details tr td:nth-child(54) input { border: 1px solid #ddd!important; outline: none; }
    #gointerdepot_sku_details tr td:nth-child(54) { width: 400px; }
    /*-----------------------
...
<div class="grn-form">
    <div class=" col-md-12">  
        <form id="form_go_inter_depot_details" action="<?php echo Url::base(); ?>index.php?r=gointerdepot%2Fsave" method="post" enctype="multipart/form-data" onkeypress="return event.keyCode != 13;">
        <!-- <form id="form_go_inter_depot_details" action="<?php //echo Url::base(); ?>index.php?r=pendinggo%2Fgetgrnparticulars" method="post"> -->
        ...
        <div class="row row-container">
            <div class="form-group">
    			...
    			<div class="col-md-2 col-sm-2 col-xs-6">
                    ...
                    <div class=" "> 
    				    <input type="text" class="form-control" name="from_state" id="from_state"   value="<?= $grn_details[0]['warehouse_state'] ?>" readonly />  
                    </div>
    			</div>
                <div class="col-md-2 col-sm-2 col-xs-6">
                    <label class=" control-label">To State </label> 
                    <div class=" "> 
                        <input type="text" class="form-control" name="to_state" id="to_state"   value="<?= $grn_details[0]['to_state'] ?>" readonly />  
                    </div>
                </div>
    	...
    	<div id="update_grn_div" class="table-container sticky-table sticky-headers sticky-ltr-cells">
            <table id="update_grn" class="table table-bordered">
                <tr class="table-head">
                    ...
                    <th class="sticky-cell" style="width: 100px; border: none!important;">Particulars</th>
                    ...
                    <?php for($i=0; $i<count($invoice_details); $i++) { ?>
                        ...
                        <th style="width: 100px;"><span>Edited</span> <br/> <span class="actual_value"> (<?php echo $invoice_details[$i]['invoice_number']; ?>) </span>  </span></th>
                        <th style="width: 100px;"><span>Difference</span> <br/> <span class="actual_value"> (<?php echo $invoice_details[$i]['invoice_number']; ?>) </span>  </span></th>
                    <?php } ?>
                    <th>Narration</th>
                </tr>

                <?php
                $taxcount = count($total_tax2);
                $total_taxable_amount = 0;

                for($j=0; $j<count($total_tax2); $j++) { ?>
                    <?php 
                        $inv_num = 0; 
                        $invoice_cost_td = ''; 
                        $invoice_tax_td = ''; 
                        ...
                        for($k=0; $k<count($invoice_details2); $k++) { 
                            $bl_invoice=false;
                            for($i=0; $i<count($invoice_tax2); $i++) { 
                                if($invoice_details2[$k]['invoice_number']==$invoice_tax2[$i]['invoice_number']) {
                                    if($total_tax2[$j]['tax_zone_code']==$invoice_tax2[$i]['tax_zone_code'] && 
                                        // $total_tax2[$j]['vat_cst'] == $invoice_tax2[$i]['vat_cst'] && 
                                        floatval($total_tax2[$j]['vat_percent']) == floatval($invoice_tax2[$i]['vat_percent'])) {

                                        $total_tax2[$j]['invoice_cost_acc_id']=$invoice_tax2[$i]['invoice_cost_acc_id'];
                                        $total_tax2[$j]['invoice_cost_ledger_name']=$invoice_tax2[$i]['invoice_cost_ledger_name'];
                                        $total_tax2[$j]['invoice_cost_ledger_code']=$invoice_tax2[$i]['invoice_cost_ledger_code'];
                                        $total_tax2[$j]['invoice_tax_acc_id']=$invoice_tax2[$i]['invoice_tax_acc_id'];
                                        $total_tax2[$j]['invoice_tax_ledger_name']=$invoice_tax2[$i]['invoice_tax_ledger_name'];
                                        $total_tax2[$j]['invoice_tax_ledger_code']=$invoice_tax2[$i]['invoice_tax_ledger_code'];
                                        $total_tax2[$j]['invoice_cgst_acc_id']=$invoice_tax2[$i]['invoice_cgst_acc_id'];
                                        $total_tax2[$j]['invoice_cgst_ledger_name']=$invoice_tax2[$i]['invoice_cgst_ledger_name'];
                                        $total_tax2[$j]['invoice_cgst_ledger_code']=$invoice_tax2[$i]['invoice_cgst_ledger_code'];
                                        $total_tax2[$j]['invoice_sgst_acc_id']=$invoice_tax2[$i]['invoice_sgst_acc_id'];
                                        $total_tax2[$j]['invoice_sgst_ledger_name']=$invoice_tax2[$i]['invoice_sgst_ledger_name'];
                                        $total_tax2[$j]['invoice_sgst_ledger_code']=$invoice_tax2[$i]['invoice_sgst_ledger_code'];
                                        $total_tax2[$j]['invoice_igst_acc_id']=$invoice_tax2[$i]['invoice_igst_acc_id'];
                                        $total_tax2[$j]['invoice_igst_ledger_name']=$invoice_tax2[$i]['invoice_igst_ledger_name'];
                                        $total_tax2[$j]['invoice_igst_ledger_code']=$invoice_tax2[$i]['invoice_igst_ledger_code'];

                                        if($invoice_tax2[$i]['invoice_cost']==0) {
                                            $total_tax2[$j]['total_cost']=$invoice_tax2[$i]['invoice_cost'];
                                            $total_tax2[$j]['total_cgst']=0;
                                            $total_tax2[$j]['total_sgst']=0;
                                            $total_tax2[$j]['total_igst']=0;
                                        } else {
                                            $total_tax2[$j]['total_cost'] = $invoice_tax2[$i]['invoice_cost'];
                                            $total_tax2[$j]['total_cgst'] = $invoice_tax2[$i]['invoice_cgst'];
                                            $total_tax2[$j]['total_sgst'] = $invoice_tax2[$i]['invoice_sgst'];
                                            $total_tax2[$j]['total_igst'] = $invoice_tax2[$i]['invoice_igst'];
                                        }

                                        $td = '<td>
                                                    <input type="text" class="text-right" id="sales_invoice_'.$k.'_cost_'.$j.'" name="sales_invoice_cost_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['invoice_cost'], 2).'" readonly />
                                                ...
                                                <td>
                                                    <input type="text" class="text-right sales_edited-cost" id="sales_edited_'.$k.'_cost_'.$j.'" name="sales_edited_cost_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['edited_cost'], 2).'" onChange="getDifference(this);" readonly />
                                                </td>
                                                <td>
                                                    <input type="text" class="text-right diff" id="sales_diff_'.$k.'_cost_'.$j.'" name="sales_diff_cost_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['diff_cost'], 2).'" readonly />
                                                </td>';
                                        $invoice_cost_td = $invoice_cost_td . $td;

                                        $td = '<td>
                                                    <input type="text" class="text-right" id="sales_invoice_'.$k.'_tax_'.$j.'" name="sales_invoice_tax_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['invoice_tax'], 2).'" readonly />
                                                ...
                                                <td style="display: none;">
                                                    <input type="text" class="text-right" id="sales_edited_'.$k.'_tax_'.$j.'" name="sales_edited_tax_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['edited_tax'], 2).'" onChange="getDifference(this);" readonly />
                                                </td>
                                                <td style="display: none;">
                                                    <input type="text" class="text-right " id="sales_diff_'.$k.'_tax_'.$j.'" name="sales_diff_tax_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['diff_tax'], 2).'" readonly />
                                                </td>';
                                        $invoice_tax_td = $invoice_tax_td . $td;

                                        $td = '<td>
                                                    <input type="text" class="text-right" id="sales_invoice_'.$k.'_cgst_'.$j.'" name="sales_invoice_cgst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['invoice_cgst'], 2).'" readonly />
                                                ...
                                                <td>
                                                    <input type="text" class="text-right" id="sales_edited_'.$k.'_cgst_'.$j.'" name="sales_edited_cgst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['edited_cgst'], 2).'" onChange="getDifference(this);" readonly />
                                                </td>
                                                <td>
                                                    <input type="text" class="text-right diff" id="sales_diff_'.$k.'_cgst_'.$j.'" name="sales_diff_cgst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['diff_cgst'], 2).'" readonly />
                                                </td>';
                                        $invoice_cgst_td = $invoice_cgst_td . $td;

                                        $td = '<td>
                                                    <input type="text" class="text-right" id="sales_invoice_'.$k.'_sgst_'.$j.'" name="sales_invoice_sgst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['invoice_sgst'], 2).'" readonly />
                                                ...
                                                <td>
                                                    <input type="text" class="text-right" id="sales_edited_'.$k.'_sgst_'.$j.'" name="sales_edited_sgst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['edited_sgst'], 2).'" onChange="getDifference(this);" readonly />
                                                </td>
                                                <td>
                                                    <input type="text" class="text-right diff" id="sales_diff_'.$k.'_sgst_'.$j.'" name="sales_diff_sgst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['diff_sgst'], 2).'" readonly />
                                                </td>';
                                        $invoice_sgst_td = $invoice_sgst_td . $td;

                                        $td = '<td>
                                                    <input type="text" class="text-right" id="sales_invoice_'.$k.'_igst_'.$j.'" name="sales_invoice_igst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['invoice_igst'], 2).'" readonly />
                                                ...
                                                <td>
                                                    <input type="text" class="text-right" id="sales_edited_'.$k.'_igst_'.$j.'" name="sales_edited_igst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['edited_igst'], 2).'" onChange="getDifference(this);" readonly />
                                                </td>
                                                <td>
                                                    <input type="text" class="text-right diff" id="sales_diff_'.$k.'_igst_'.$j.'" name="sales_diff_igst_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax2[$i]['diff_igst'], 2).'" readonly />
                                                </td>';
                            ...
                            if($bl_invoice==false) {
                                $td = '<td>
                                            <input type="text" class="text-right" id="sales_invoice_'.$inv_num.'_cost_'.$j.'" name="sales_invoice_cost_'.$j.'[]" value="0.00" readonly />
                                            <input type="hidden" id="sales_invoice_'.$k.'_cost_voucher_id_'.$j.'" name="sales_invoice_cost_voucher_id_'.$j.'[]" value="" />
                                            <input type="hidden" id="sales_invoice_'.$k.'_cost_ledger_type_'.$j.'" name="sales_invoice_cost_ledger_type_'.$j.'[]" value="Sub Entry" />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right sales_edited-cost" id="sales_edited_'.$inv_num.'_cost_'.$j.'" name="sales_edited_cost_'.$j.'[]" value="0.00" onChange="getDifference(this);" readonly />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right diff" id="sales_diff_'.$inv_num.'_cost_'.$j.'" name="sales_diff_cost_'.$j.'[]" value="0.00" readonly />
                                        </td>';
                                $invoice_cost_td = $invoice_cost_td . $td;

                                $td = '<td>
                                            <input type="text" class="text-right" id="sales_invoice_'.$inv_num.'_tax_'.$j.'" name="sales_invoice_tax_'.$j.'[]" value="0.00" readonly />
                                            <input type="hidden" id="sales_invoice_'.$k.'_tax_voucher_id_'.$j.'" name="sales_invoice_tax_voucher_id_'.$j.'[]" value="" />
                                            <input type="hidden" id="sales_invoice_'.$k.'_tax_ledger_type_'.$j.'" name="sales_invoice_tax_ledger_type_'.$j.'[]" value="Sub Entry" />
                                        </td>
                                        <td style="display: none;">
                                            <input type="text" class="text-right" id="sales_edited_'.$inv_num.'_tax_'.$j.'" name="sales_edited_tax_'.$j.'[]" value="0.00" onChange="getDifference(this);" readonly />
                                        </td>
                                        <td style="display: none;">
                                            <input type="text" class="text-right " id="sales_diff_'.$inv_num.'_tax_'.$j.'" name="sales_diff_tax_'.$j.'[]" value="0.00" readonly />
                                        </td>';
                                $invoice_tax_td = $invoice_tax_td . $td;
                                
                                $td = '<td>
                                            <input type="text" class="text-right" id="sales_invoice_'.$inv_num.'_cgst_'.$j.'" name="sales_invoice_cgst_'.$j.'[]" value="0.00" readonly />
                                            <input type="hidden" id="sales_invoice_'.$k.'_cgst_voucher_id_'.$j.'" name="sales_invoice_cgst_voucher_id_'.$j.'[]" value="" />
                                            <input type="hidden" id="sales_invoice_'.$k.'_cgst_ledger_type_'.$j.'" name="sales_invoice_cgst_ledger_type_'.$j.'[]" value="Sub Entry" />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right" id="sales_edited_'.$inv_num.'_cgst_'.$j.'" name="sales_edited_cgst_'.$j.'[]" value="0.00" onChange="getDifference(this);" readonly />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right diff" id="sales_diff_'.$inv_num.'_cgst_'.$j.'" name="sales_diff_cgst_'.$j.'[]" value="0.00" readonly />
                                        </td>';
                                $invoice_cgst_td = $invoice_cgst_td . $td;
                                
                                $td = '<td>
                                            <input type="text" class="text-right" id="sales_invoice_'.$inv_num.'_sgst_'.$j.'" name="sales_invoice_sgst_'.$j.'[]" value="0.00" readonly />
                                            <input type="hidden" id="sales_invoice_'.$k.'_sgst_voucher_id_'.$j.'" name="sales_invoice_sgst_voucher_id_'.$j.'[]" value="" />
                                            <input type="hidden" id="sales_invoice_'.$k.'_sgst_ledger_type_'.$j.'" name="sales_invoice_sgst_ledger_type_'.$j.'[]" value="Sub Entry" />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right" id="sales_edited_'.$inv_num.'_sgst_'.$j.'" name="sales_edited_sgst_'.$j.'[]" value="0.00" onChange="getDifference(this);" readonly />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right diff" id="sales_diff_'.$inv_num.'_sgst_'.$j.'" name="sales_diff_sgst_'.$j.'[]" value="0.00" readonly />
                                        </td>';
                                $invoice_sgst_td = $invoice_sgst_td . $td;
                                
                                $td = '<td>
                                            <input type="text" class="text-right" id="sales_invoice_'.$inv_num.'_igst_'.$j.'" name="sales_invoice_igst_'.$j.'[]" value="0.00" readonly />
                                            <input type="hidden" id="sales_invoice_'.$k.'_igst_voucher_id_'.$j.'" name="sales_invoice_igst_voucher_id_'.$j.'[]" value="" />
                                            <input type="hidden" id="sales_invoice_'.$k.'_igst_ledger_type_'.$j.'" name="sales_invoice_igst_ledger_type_'.$j.'[]" value="Sub Entry" />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right" id="sales_edited_'.$inv_num.'_igst_'.$j.'" name="sales_edited_igst_'.$j.'[]" value="0.00" onChange="getDifference(this);" readonly />
                                        </td>
                                        <td>
                                            <input type="text" class="text-right diff" id="sales_diff_'.$inv_num.'_igst_'.$j.'" name="sales_diff_igst_'.$j.'[]" value="0.00" readonly />
                                        </td>';
                                $invoice_igst_td = $invoice_igst_td . $td;
                        ...
                        $total_tax2[$j]['invoice_cost_td']=$invoice_cost_td;
                        $total_tax2[$j]['invoice_tax_td']=$invoice_tax_td;
                        $total_tax2[$j]['invoice_cgst_td']=$invoice_cgst_td;
                        $total_tax2[$j]['invoice_sgst_td']=$invoice_sgst_td;
                        $total_tax2[$j]['invoice_igst_td']=$invoice_igst_td;
                    ?>
                    ...
                    <tr>
                        ...
                        <td class="sticky-cell" style="border: none!important;">
                            <select id="salesinvoicecost_acc_id_<?php echo $j;?>" class="form-control acc_id select2" name="sales_invoice_cost_acc_id[]" onChange="get_acc_details(this)">
                                <option value="">Select</option>
                                <?php for($i=0; $i<count($acc_master); $i++) { 
                                        if($acc_master[$i]['type']=="Goods Sales") { 
                                ?>
                                <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($total_tax2[$j]['invoice_cost_acc_id']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                                <?php }} ?>
                            </select>
                            <input type="hidden" id="salesinvoicecost_ledger_name_<?php echo $j;?>" name="sales_invoice_cost_ledger_name[]" value="<?php echo $total_tax2[$j]['invoice_cost_ledger_name']; ?>" />
                        </td>
                        <td class="sticky-cell" style="border: none!important;"><input type="text" id="salesinvoicecost_ledger_code_<?php echo $j;?>" name="sales_invoice_cost_ledger_code[]" value="<?php echo $total_tax2[$j]['invoice_cost_ledger_code']; ?>" style="border: none;" readonly /></td>
                        <td class="sticky-cell" style="border: none!important;">
                            <input type="hidden" id="sales_vat_cst_<?php echo $j;?>" name="sales_vat_cst[]" value="<?php echo $total_tax2[$j]['vat_cst']; ?>" />
                            <input type="hidden" id="sales_vat_percen_<?php echo $j;?>" name="sales_vat_percen[]" value="<?php echo $total_tax2[$j]['vat_percent']; ?>" />
                            <input type="hidden" id="sales_cgst_rate_<?php echo $j;?>" name="sales_cgst_rate[]" value="<?php echo $total_tax2[$j]['cgst_rate']; ?>" />
                            <input type="hidden" id="sales_sgst_rate_<?php echo $j;?>" name="sales_sgst_rate[]" value="<?php echo $total_tax2[$j]['sgst_rate']; ?>" />
                            <input type="hidden" id="sales_igst_rate_<?php echo $j;?>" name="sales_igst_rate[]" value="<?php echo $total_tax2[$j]['igst_rate']; ?>" />
                            <input type="hidden" id="sales_sub_particular_cost_<?php echo $j;?>" name="sales_sub_particular_cost[]" value="<?php echo 'Purchase_'.$total_tax2[$j]['tax_zone_code'].'_'.$total_tax2[$j]['vat_cst'].'_'.$total_tax2[$j]['vat_percent']; ?>" />
                        ...
                        <?php $total_taxable_amount = $total_taxable_amount + $total_tax2[$j]['total_cost']; ?>
                        <?php echo $total_tax2[$j]['invoice_cost_td']; ?>
                        <td>
                            <input type="text" id="sales_narration_cost_<?php echo $j;?>" name="sales_narration_cost_<?php echo $j;?>" value="<?php echo $narration2[$j]['cost']; ?>" class="narration"/>
                        </td>
                    </tr>
                    <tr style="display: none;">
                        ...
                        <td class="sticky-cell" style="border: none!important;">
                            <select id="salesinvoicetax_acc_id_<?php echo $j;?>" class="form-control acc_id select2" name="sales_invoice_tax_acc_id[]" onChange="get_acc_details(this)">
                                <option value="">Select</option>
                                <?php for($i=0; $i<count($acc_master); $i++) { 
                                        if($acc_master[$i]['type']=="Tax") { 
                                ?>
                                <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($total_tax2[$j]['invoice_tax_acc_id']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                                <?php }} ?>
                            </select>
                            <input type="hidden" id="salesinvoicetax_ledger_name_<?php echo $j;?>" name="sales_invoice_tax_ledger_name[]" value="<?php echo $total_tax2[$j]['invoice_tax_ledger_name']; ?>" />
                        </td>
                        <td class="sticky-cell" style="border: none!important;"><input type="text" id="salesinvoicetax_ledger_code_<?php echo $j;?>" name="sales_invoice_tax_ledger_code[]" value="<?php echo $total_tax2[$j]['invoice_tax_ledger_code']; ?>" style="border: none;" readonly /></td>
                        <td class="sticky-cell" style="border: none!important;">
                            <!-- <input type="hidden" id="sales_vat_cst_<?php //echo $j;?>" name="sales_vat_cst[]" value="<?php //echo $total_tax2[$j]['vat_cst']; ?>" />
                            <input type="hidden" id="sales_vat_percen_<?php //echo $j;?>" name="sales_vat_percen[]" value="<?php //echo $total_tax2[$j]['vat_percen']; ?>" /> -->
                            <input type="hidden" id="sales_sub_particular_tax_<?php echo $j;?>" name="sales_sub_particular_tax[]" value="<?php echo 'Tax_'.$total_tax2[$j]['tax_zone_code'].'_'.$total_tax2[$j]['vat_percent']; ?>" />
                        ...
                        <?php echo $total_tax2[$j]['invoice_tax_td']; ?>
                        <td>
                            <input type="text" id="sales_narration_tax_<?php echo $j;?>" name="sales_narration_tax_<?php echo $j;?>" value="<?php echo $narration2[$j]['tax']; ?>" class="narration"/>
                        </td>
                    </tr>
                    <tr style="<?php echo $intra_state_style; ?>">
                        ...
                        <td class="sticky-cell" style="border: none!important;">
                            <select id="salesinvoicecgst_acc_id_<?php echo $j;?>" class="form-control acc_id select2" name="sales_invoice_cgst_acc_id[]" onChange="get_acc_details(this)">
                                <option value="">Select</option>
                                <?php for($i=0; $i<count($acc_master); $i++) { 
                                        if($acc_master[$i]['type']=="CGST") { 
                                ?>
                                <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($total_tax2[$j]['invoice_cgst_acc_id']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                                <?php }} ?>
                            </select>
                            <input type="hidden" id="salesinvoicecgst_ledger_name_<?php echo $j;?>" name="sales_invoice_cgst_ledger_name[]" value="<?php echo $total_tax2[$j]['invoice_cgst_ledger_name']; ?>" />
                        </td>
                        <td class="sticky-cell" style="border: none!important;"><input type="text" id="salesinvoicecgst_ledger_code_<?php echo $j;?>" name="sales_invoice_cgst_ledger_code[]" value="<?php echo $total_tax2[$j]['invoice_cgst_ledger_code']; ?>" style="border: none;" readonly /></td>
                        <td class="sticky-cell" style="border: none!important;">
                            <!-- <input type="hidden" id="sales_vat_cst_<?php //echo $j;?>" name="sales_vat_cst[]" value="<?php //echo $total_tax2[$j]['vat_cst']; ?>" />
                            <input type="hidden" id="sales_vat_percen_<?php //echo $j;?>" name="sales_vat_percen[]" value="<?php //echo $total_tax2[$j]['cgst']; ?>" /> -->
                            <input type="hidden" id="sales_sub_particular_cgst_<?php echo $j;?>" name="sales_sub_particular_cgst[]" value="<?php echo 'Tax_cgst_'.$total_tax2[$j]['cgst_rate']; ?>" />
                        ...
                        <?php echo $total_tax2[$j]['invoice_cgst_td']; ?>
                        <td>
                            <input type="text" id="sales_narration_cgst_<?php echo $j;?>" name="sales_narration_cgst_<?php echo $j;?>" value="<?php echo $narration2[$j]['cgst']; ?>" class="narration"/>
                        </td>
                    </tr>
                    <tr style="<?php echo $intra_state_style; ?>">
                        <td class="sticky-cell" style="border: none!important;"><?php echo '2.'.($j+1); ?></td>
                        ...
                        <td class="sticky-cell" style="border: none!important;">
                            <select id="salesinvoicesgst_acc_id_<?php echo $j;?>" class="form-control acc_id select2" name="sales_invoice_sgst_acc_id[]" onChange="get_acc_details(this)">
                                <option value="">Select</option>
                                <?php for($i=0; $i<count($acc_master); $i++) { 
                                        if($acc_master[$i]['type']=="SGST") { 
                                ?>
                                <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($total_tax2[$j]['invoice_sgst_acc_id']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                                <?php }} ?>
                            </select>
                            <input type="hidden" id="salesinvoicesgst_ledger_name_<?php echo $j;?>" name="sales_invoice_sgst_ledger_name[]" value="<?php echo $total_tax2[$j]['invoice_sgst_ledger_name']; ?>" />
                        </td>
                        <td class="sticky-cell" style="border: none!important;"><input type="text" id="salesinvoicesgst_ledger_code_<?php echo $j;?>" name="sales_invoice_sgst_ledger_code[]" value="<?php echo $total_tax2[$j]['invoice_sgst_ledger_code']; ?>" style="border: none;" readonly /></td>
                        <td class="sticky-cell" style="border: none!important;">
                            <!-- <input type="hidden" id="sales_vat_cst_<?php //echo $j;?>" name="sales_vat_cst[]" value="<?php //echo $total_tax2[$j]['vat_cst']; ?>" />
                            <input type="hidden" id="sales_vat_percen_<?php //echo $j;?>" name="sales_vat_percen[]" value="<?php //echo $total_tax2[$j]['sgst']; ?>" /> -->
                            <input type="hidden" id="sales_sub_particular_sgst_<?php echo $j;?>" name="sales_sub_particular_sgst[]" value="<?php echo 'Tax_sgst_'.$total_tax2[$j]['sgst_rate']; ?>" />
                        ...
                        <?php echo $total_tax2[$j]['invoice_sgst_td']; ?>
                        <td>
                            <input type="text" id="sales_narration_sgst_<?php echo $j;?>" name="sales_narration_sgst_<?php echo $j;?>" value="<?php echo $narration2[$j]['sgst']; ?>" class="narration"/>
                        </td>
                    </tr>
                    <tr style="<?php echo $inter_state_style; ?>">
                        ...
                        <td class="sticky-cell" style="border: none!important;">
                            <select id="salesinvoiceigst_acc_id_<?php echo $j;?>" class="form-control acc_id select2" name="sales_invoice_igst_acc_id[]" onChange="get_acc_details(this)">
                                <option value="">Select</option>
                                <?php for($i=0; $i<count($acc_master); $i++) { 
                                        if($acc_master[$i]['type']=="IGST") { 
                                ?>
                                <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($total_tax2[$j]['invoice_igst_acc_id']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                                <?php }} ?>
                            </select>
                            <input type="hidden" id="salesinvoiceigst_ledger_name_<?php echo $j;?>" name="sales_invoice_igst_ledger_name[]" value="<?php echo $total_tax2[$j]['invoice_igst_ledger_name']; ?>" />
                        </td>
                        <td class="sticky-cell" style="border: none!important;"><input type="text" id="salesinvoiceigst_ledger_code_<?php echo $j;?>" name="sales_invoice_igst_ledger_code[]" value="<?php echo $total_tax2[$j]['invoice_igst_ledger_code']; ?>" style="border: none;" readonly /></td>
                        <td class="sticky-cell" style="border: none!important;">
                            <!-- <input type="hidden" id="sales_vat_cst_<?php //echo $j;?>" name="sales_vat_cst[]" value="<?php //echo $total_tax2[$j]['vat_cst']; ?>" />
                            <input type="hidden" id="sales_vat_percen_<?php //echo $j;?>" name="sales_vat_percen[]" value="<?php //echo $total_tax2[$j]['igst']; ?>" /> -->
                            <input type="hidden" id="sales_sub_particular_igst_<?php echo $j;?>" name="sales_sub_particular_igst[]" value="<?php echo 'Tax_igst_'.$total_tax2[$j]['igst_rate']; ?>" />
                        ...
                        <?php echo $total_tax2[$j]['invoice_igst_td']; ?>
                        <td>
                            <input type="text" class="narration" id="sales_narration_igst_<?php echo $j;?>" name="sales_narration_igst_<?php echo $j;?>" value="<?php echo $narration2[$j]['igst']; ?>" />
                        </td>
                    </tr>
                <?php } ?>

                <tr id="sales_othercharges">
                    <td class="sticky-cell" style="border: none!important;">3</td>
                    <td class="sticky-cell" style="border: none!important;">Other Charges</td>
                    <td class="sticky-cell" style="border: none!important;">
                        <select id="salesothercharges_acc_id_0" class="form-control acc_id select2" name="sales_other_charges_acc_id" onChange="get_acc_details(this)">
                            <option value="">Select</option>
                            <?php for($i=0; $i<count($acc_master); $i++) { 
                                    if($acc_master[$i]['type']=="Others") { 
                            ?>
                            <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($acc['other_charges_acc_id2']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                            <?php }} ?>
                        </select>
                        <input type="hidden" id="salesothercharges_ledger_name_0" name="sales_other_charges_ledger_name" value="<?php echo $acc['other_charges_ledger_name2']; ?>" />
                    </td>
                    <td class="sticky-cell" style="border: none!important;">
                    ...
                    <?php for($i=0; $i<count($invoice_details2); $i++) { ?>
                        <td>
                            <input type="text" class="text-right" id="sales_invoice_other_charges_<?php echo $i;?>" name="sales_invoice_other_charges[]" value="<?php echo $mycomponent->format_money($invoice_details2[$i]['invoice_other_charges'], 2); ?>" readonly />
                        </td>
                        <td>
                            <input type="text" class="text-right edit-text sales_edited_other_charges diff" id="sales_edited_other_charges_<?php echo $i;?>" name="sales_edited_other_charges[]" value="<?php echo $mycomponent->format_money($invoice_details2[$i]['edited_other_charges'], 2); ?>" onChange="getDifference(this);" />
                        </td>
                        <td>
                            <input type="text" class="text-right" id="sales_diff_other_charges_<?php echo $i;?>" name="sales_diff_other_charges[]" value="<?php echo $mycomponent->format_money($invoice_details2[$i]['diff_other_charges'], 2); ?>" readonly />
                        </td>
                    <?php } ?>
                    <td>
                        <input type="text"  id="sales_narration_other_charges" name="sales_narration_other_charges" value="<?php echo $narration2['narration_other_charges']; ?>" />
                    </td>
                </tr>
                <tr class="bold-text" style=" ">
                    <td class="sticky-cell" style="border: none!important; background-color: #f9f9f9;"><button type="button" class="btn btn-info btn-xs  " id="get_gointerdepot_qty">Edit</button></td>
                    ...
                    <td class="sticky-cell" style="border: none!important; background-color: #f9f9f9;">
                        ...
                        <?php if($grn_details[0]['warehouse_state']!=""){ ?>
                            <select id="salestotalamount_acc_id_0" class="form-control acc_id " name="sales_total_amount_acc_id" onChange="get_acc_details(this)" style="display: none;">
                                ...
                                ?>
                                        <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($acc['total_amount_acc_id2']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                                <?php }} ?>
                            </select>
                            <input type="hidden" id="salestotalamount_ledger_name_0" name="sales_total_amount_ledger_name" value="<?php echo $acc['total_amount_ledger_name2']; ?>" />
                        
                        <?php echo $acc['total_amount_ledger_name2']; ?>

                        <?php } else { ?>

                            <select id="salestotalamount_acc_id_0" class="form-control acc_id " name="sales_total_amount_acc_id" onChange="get_acc_details(this)" >
                                <option value="">Select</option>
                                <?php 
                                for($i=0; $i<count($acc_master); $i++) { 
                                    if($acc_master[$i]['type']=="Branch Type") { 
                                ?>
                                    <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($ware_array2['total_amount_acc_id2']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                                <?php }} ?>
                            </select>

                            <input type="hidden" id="salestotalamount_ledger_name_0" name="sales_total_amount_ledger_name" value="<?php echo $ware_array2['total_amount_ledger_name2']; ?>" />
                            <?php echo $ware_array2['total_amount_ledger_name2']; ?>
                    ...
                    <?php for($i=0; $i<count($invoice_details2); $i++) { ?>
                        <td>
                            <input type="text" class="text-right" id="sales_invoice_total_amount_<?php echo $i;?>" name="sales_invoice_total_amount[]" value="<?php echo $mycomponent->format_money($invoice_details2[$i]['invoice_total_amount'], 2); ?>" readonly />
                            <input type="hidden" id="sales_total_amount_voucher_id_<?php echo $i;?>" name="sales_total_amount_voucher_id[]" value="<?php echo $invoice_details2[$i]['total_amount_voucher_id']; ?>" />
                            <input type="hidden" id="sales_total_amount_ledger_type_<?php echo $i;?>" name="sales_total_amount_ledger_type[]" value="<?php echo $invoice_details2[$i]['total_amount_ledger_type']; ?>" />
                        </td>
                        <td>
                            <input type="text" class="text-right" id="sales_edited_total_amount_<?php echo $i;?>" name="sales_edited_total_amount[]" value="<?php echo $mycomponent->format_money($invoice_details2[$i]['edited_total_amount'], 2); ?>" onChange="getDifference(this);" readonly />
                        </td>
                        <td>
                            <input type="text" class="text-right" id="sales_diff_total_amount_<?php echo $i;?>" name="sales_diff_total_amount[]" value="<?php echo $mycomponent->format_money($invoice_details2[$i]['diff_total_amount'], 2); ?>" readonly />
                        </td>
                    <?php } ?>
                    <td>
                        <input type="text" id="sales_narration_total_amount" name="sales_narration_total_amount" value="<?php echo $narration2['narration_total_amount']; ?>" />
                    </td>
                </tr>
                <tr class="bold-text" style="display: none;">
                    <td class="sticky-cell" style="border: none!important; background-color: #f9f9f9;"></td>
                    ...
                    <?php for($i=0; $i<count($invoice_details2); $i++) { ?>
                        ...
                        <td>
                            <input type="text" class="text-right total-amount" id="sales_edited_total_payable_amount_<?php echo $i;?>" name="sales_edited_total_payable_amount[]" value="<?php //echo $mycomponent->format_money($invoice_details2[$i]['edited_total_payable_amount'], 2); ?>" readonly />
                        </td>
                        <td id="sales_diff_total_payable_amount_<?php echo $i;?>" class="text-right">
                        ...
                        for($k=0; $k<count($invoice_details); $k++) { 
                                ...
                                if($invoice_details[$k]['invoice_number']==$invoice_tax[$i]['invoice_number']) {
                                    if($total_tax[$j]['tax_zone_code']==$invoice_tax[$i]['tax_zone_code'] && 
                                        ...
                                        $td = '<td>
                                                ...
                                                <td>
                                                    <input type="text" class="text-right edited-cost" id="edited_'.$k.'_cost_'.$j.'" name="edited_cost_'.$j.'[]" value="'.$mycomponent->format_money($invoice_tax[$i]['edited_cost'], 2).'" onChange="getDifference(this);" readonly />
                                                </td>
                                                <td>
                            ...
                            if($bl_invoice==false) {
                                $td = '<td>
                                        ...
                                        <td>
                                            <input type="text" class="text-right edited-cost" id="edited_'.$inv_num.'_cost_'.$j.'" name="edited_cost_'.$j.'[]" value="0.00" onChange="getDifference(this);" readonly />
                                        </td>
                                        <td>
                ...
                <tr class="bold-text" style=" ">
                    ...
                    <td class="sticky-cell" style="border: none!important; background-color: #f9f9f9;">
                        ...
                        <?php echo $ware_array['total_amount_ledger_name']; ?>
                ...
                <tr id="salesstocktransfer">
                    <td class="sticky-cell" style="border: none!important;">1</td>
                    <td class="sticky-cell" style="border: none!important;">Sales Stock Transfer</td>
                    <td class="sticky-cell" style="border: none!important;">
                        <select id="salesstocktransfer_acc_id_0" class="form-control acc_id select2" name="sales_stock_transfer_acc_id" onChange="get_acc_details(this)">
                            <option value="">Select</option>
                            <?php for($i=0; $i<count($acc_master); $i++) { 
                                    if($acc_master[$i]['type']=="Others") { 
                            ?>
                            <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($acc['sales_stock_transfer_acc_id']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                            <?php }} ?>
                        </select>
                        <input type="hidden" id="salesstocktransfer_ledger_name_0" name="sales_stock_transfer_ledger_name" value="<?php echo $acc['sales_stock_transfer_ledger_name']; ?>" />
                    </td>
                    <td class="sticky-cell" style="border: none!important;">
                    ...
                    <?php for($i=0; $i<count($invoice_details); $i++) { ?>
                        <td>
                            <input type="text" class="text-right" id="invoice_sales_stock_transfer_<?php echo $i;?>" name="invoice_sales_stock_transfer[]" value="<?php echo $mycomponent->format_money($invoice_details[$i]['invoice_sales_stock_transfer'], 2); ?>" readonly />
                            <input type="hidden" id="sales_stock_transfer_voucher_id_<?php echo $i;?>" name="sales_stock_transfer_voucher_id[]" value="<?php echo $invoice_details[$i]['sales_stock_transfer_voucher_id']; ?>" />
                            <input type="hidden" id="sales_stock_transfer_ledger_type_<?php echo $i;?>" name="sales_stock_transfer_ledger_type[]" value="<?php echo $invoice_details[$i]['sales_stock_transfer_ledger_type']; ?>" />
                        </td>
                        <td>
                            <input type="text" class="text-right edited_sales_stock_transfer diff" id="edited_sales_stock_transfer_<?php echo $i;?>" name="edited_sales_stock_transfer[]" value="<?php echo $mycomponent->format_money($invoice_details[$i]['edited_sales_stock_transfer'], 2); ?>" onChange="getDifference(this);" readonly />
                        </td>
                        <td>
                            <input type="text" class="text-right" id="diff_sales_stock_transfer_<?php echo $i;?>" name="diff_sales_stock_transfer[]" value="<?php echo $mycomponent->format_money($invoice_details[$i]['diff_sales_stock_transfer'], 2); ?>" readonly />
                        </td>
                    <?php } ?>
                    <td>
                        <input type="text"  id="narration_sales_stock_transfer" name="narration_sales_stock_transfer" value="<?php echo $narration['narration_sales_stock_transfer']; ?>" />
                    </td>
                </tr>
                <tr id="purchasestocktransfer">
                    <td class="sticky-cell" style="border: none!important;">2</td>
                    <td class="sticky-cell" style="border: none!important;">Purchase Stock Transfer</td>
                    <td class="sticky-cell" style="border: none!important;">
                        <select id="purchasestocktransfer_acc_id_0" class="form-control acc_id select2" name="purchase_stock_transfer_acc_id" onChange="get_acc_details(this)">
                            <option value="">Select</option>
                            <?php for($i=0; $i<count($acc_master); $i++) { 
                                    if($acc_master[$i]['type']=="Others") { 
                            ?>
                            <option value="<?php echo $acc_master[$i]['id']; ?>" <?php if($acc['purchase_stock_transfer_acc_id']==$acc_master[$i]['id']) echo 'selected'; ?>><?php echo $acc_master[$i]['legal_name']; ?></option>
                            <?php }} ?>
                        </select>
                        <input type="hidden" id="purchasestocktransfer_ledger_name_0" name="purchase_stock_transfer_ledger_name" value="<?php echo $acc['purchase_stock_transfer_ledger_name']; ?>" />
                    </td>
                    <td class="sticky-cell" style="border: none!important;">
                    ...
                    <?php for($i=0; $i<count($invoice_details); $i++) { ?>
                        <td>
                            <input type="text" class="text-right" id="invoice_purchase_stock_transfer_<?php echo $i;?>" name="invoice_purchase_stock_transfer[]" value="<?php echo $mycomponent->format_money($invoice_details[$i]['invoice_purchase_stock_transfer'], 2); ?>" readonly />
                            <input type="hidden" id="purchase_stock_transfer_voucher_id_<?php echo $i;?>" name="purchase_stock_transfer_voucher_id[]" value="<?php echo $invoice_details[$i]['purchase_stock_transfer_voucher_id']; ?>" />
                            <input type="hidden" id="purchase_stock_transfer_ledger_type_<?php echo $i;?>" name="purchase_stock_transfer_ledger_type[]" value="<?php echo $invoice_details[$i]['purchase_stock_transfer_ledger_type']; ?>" />
                        </td>
                        <td>
                            <input type="text" class="text-right edited_purchase_stock_transfer diff" id="edited_purchase_stock_transfer_<?php echo $i;?>" name="edited_purchase_stock_transfer[]" value="<?php echo $mycomponent->format_money($invoice_details[$i]['edited_purchase_stock_transfer'], 2); ?>" onChange="getDifference(this);" readonly />
                        </td>
                        <td>
                            <input type="text" class="text-right" id="diff_purchase_stock_transfer_<?php echo $i;?>" name="diff_purchase_stock_transfer[]" value="<?php echo $mycomponent->format_money($invoice_details[$i]['diff_purchase_stock_transfer'], 2); ?>" readonly />
                        </td>
                    <?php } ?>
                    <td>
                        <input type="text"  id="narration_purchase_stock_transfer" name="narration_purchase_stock_transfer" value="<?php echo $narration['narration_purchase_stock_transfer']; ?>" />
                    </td>
                </tr>
            </table>
         </div>
        ...
        <!-- Go Inter Depot Modal -->
        <div class="modal fade" id="gointerdepot_modal" role="dialog">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body" style=" ">
                       ...
                       <table class="table table-bordered" id="gointerdepot_sku_details">
                            <thead>
                                <tr>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default btn-danger" id="close_gointerdepot_modal">Save</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="form-group btn-margin">
            ...
            <a class="btn btn-sm btn-danger pull-right" href="<?php echo Url::base(); ?>index.php?r=gointerdepot%2Findex">Close</a>
...
<?php 
    ...
    $this->registerJsFile(
        '@web/js/go_inter_depot_details.js',
        ['depends' => [\yii\web\JqueryAsset::className()]]
    );
?>