...
<head>
    ...
    <title> Primarc Pecan | <?php if(isset($debit_note[0]['trans_type'])) echo $debit_note[0]['trans_type']; ?> Note</title>
...
<div class="debit_note">
    ...
    <table width="100%" border="0" cellspacing="0" class="table" style="border-collapse:collapse;  ">
        ...
        <tr style="border:none;">
            ...
            <td width="40%" style="border:none;"><p><b> <?php if(isset($debit_note[0]['debit_credit_note_ref'])) echo $debit_note[0]['debit_credit_note_ref']; ?> </b></p></td>
        ...
        <tr style="border:none;">
            ...
            <td width="3%" style="border:none;">:</td>
            <td width="40%" style="border:none;"><p><b> <?php if(isset($debit_note[0]['reference'])) echo $debit_note[0]['reference']; ?> </b></p></td>
    ...
    <table width="100%" border="1" cellspacing="0" class="table" style="border-collapse:collapse;border-color:#999;width:500px;  ">
        <tr>
           ...
           <?php 
                        $sum = 0;
                        for($i=0; $i<count($ledger_details); $i++) { ?>
         <tr valign="top">
            <td colspan="3"   style=" ">
                <p>
                     <?php if(isset($ledger_details[$i]['account_name'])) echo $ledger_details[$i]['account_name']; ?> 
                </p>
            </td>
            <td colspan="3" align="center" valign="top" style=""><p><b>
            
            <?php if(isset($ledger_details[$i]['transaction'])) if($ledger_details[$i]['transaction']=='Credit')
            {
                if(isset($ledger_details[$i]['credit_amt'])) echo $mycomponent->format_money($ledger_details[$i]['credit_amt'],2);
            }
            else
            {
                if(isset($ledger_details[$i]['debit_amt'])) echo $mycomponent->format_money($ledger_details[$i]['debit_amt'],2);
            }
            ?></b></p></td>
                        </tr><?php  $sum += ($ledger_details[$i]['debit_amt'])+($ledger_details[$i]['credit_amt']) ;} ?>
        <tr>
