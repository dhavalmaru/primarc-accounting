...
class PaymentReceipt extends Model
{
    ...
    public function getPayment_upload()
    {
        $session = Yii::$app->session;
        $company_id = $session['company_id'];
        $sql = "select * from  acc_payment_upload A
        left join user B on (A.uploaded_by = B.id) 
        left join user C on (A.uploaded_by = C.id) 
    ...
    public function getLedger($id, $acc_id,$to_date=''){
        $status = 'approved';
        ...
        if($to_date!='')
            $cond = " and date(A.ref_date)<'$to_date'";
