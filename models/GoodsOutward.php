...
class GoodsOutward extends Model
{
    ...
    public function getNewGoDetails() {
        ...
        $sql = "select *, (SELECT sum(ifnull(round(value_at_cost*vat_percent/100,2),0)+value_at_cost )as total_amount from prepare_go_items Where prepare_go_id=C.pre_go_ref) as total_amount  from 
    ...
    public function getGrnAccEntries($id){
        ...
        $sql = "Select A.*, B.*,A.vendor_id as vendor_id1 from (
                ...
                AND A.inward_outward = 'outward' and A.type_outward in ('VENDOR', 'INTER-DEPOT') ) A
                Left Join
    ...
    public function getGrnDetails($id){
        ...
        $sql = "Select A.* from (
                ...
                Case When A.warehouse_state=H.to_state Then 'INTRA' Else 'INTER' end as  tax_zone_code ,A.*, A.vendor_id as vendor_id1,H.to_state,E.idt_warehouse,E.warehouse_id
                from goods_inward_outward A
                left join (Select (Case When type_outward='VENDOR' Then vendor_state When  type_outward='INTER-DEPOT' Then idt_warehouse_state end) as to_state,gi_go_id from  goods_inward_outward ) H on(A.gi_go_id = H.gi_go_id) 
                left join 
                (select distinct warehouse_name as idt_warehouse,id as warehouse_id , warehouse_code from internal_warehouse_master) E
                on (A.idt_warehouse_code = E.warehouse_code)
                left join acc_go_debit_entries C on (A.gi_go_id = C.gi_go_id and C.status = 'approved' and C.is_active='1' and 
                C.particular = 'Total Amount')
                where  date(A.gi_go_date_time) > date('2017-07-01') and A.company_id='$company_id' and
                A.inward_outward='OUTWARD' and A.type_outward in ('VENDOR', 'INTER-DEPOT')  AND A.company_id='$company_id' AND A.gi_go_id=$id 
                ) A
                ";         
    ...
    public function getGrnPostingDetails($id ,$skuentries){
            ...
            //echo $skuentries;
            if($skuentries)
                $fromtable = 'prepare_go_sku_items';
            else
            ...
            $sql = "Select AA.*,BB.tax_zone_code, BB.tax_zone_name, BB.cgst_rate, BB.sgst_rate, BB.igst_rate, 
                ...
                (ifnull(round(AA.value_at_cost*AA.vat_percent/100,2),0)+value_at_cost) as total_amount, per_unit as per_unit_exc_tax from (
                 SELECT  A.*,B.cost as per_unit,product_title,mrp,psku,fnsku,hsn_code,batch_code,asin,expiry_date,ean,sku_code,grn_no,
                 B.shipment_plan_name,B.isa,B.po_no,B.go_no,B.grn_entries_id,B.product_id,B.is_combo_items,B.order_qty,B.manual_discount,
                 B.value_at_mrp,B.vat_percent,B.quantity as invoice_qty,
                 B.value_at_cost as cost_excl_vat from 
                (Select * from 
                ...
                left join
                (Select * from  prepare_go_sku_items Union  Select * from  prepare_go_items) B on A.pre_go_ref=B.prepare_go_id 
                )AA
                left join
    ...
    public function getGrnskuDetails($id ,$skuentries){
            ...
            $sql = "Select AA.*,BB.tax_zone_code, BB.tax_zone_name, BB.cgst_rate, BB.sgst_rate, BB.igst_rate, 
                ...
                A.pre_go_ref=B.prepare_go_id )A  
                left join
                (Select * from $fromtable) B on A.pre_go_ref=B.prepare_go_id 
                )AA
                left join
                (select id, tax_zone_code, tax_zone_name, parent_id, tax_rate, 
                                    max(case when child_tax_type_code = 'CGST' then child_tax_rate else 0 end) as cgst_rate, 
    ...
    {
        $sql = "SELECT count(*) as skucount  from prepare_go_sku_items Where prepare_go_id=$prepare_go_id";
        $command = Yii::$app->db->createCommand($sql);
        $reader = $command->query();
        return $reader->readAll();
    }
    ...
    { 
       ...
       for($i=0; $i<count($ean); $i++){
        ...
        if(count($bulkInsertArray)>0){
                $sql = "delete from prepare_go_sku_items where prepare_go_id = '$prepare_go_id[0]'";
                Yii::$app->db->createCommand($sql)->execute();
                ...
                $tableName = "prepare_go_sku_items";
