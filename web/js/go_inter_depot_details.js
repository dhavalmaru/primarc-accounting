...
$(document).ready(function(){
    ...
    if($('#salestotalamount_acc_id_0').val()==""){
        alert('State Account code for '+$('#from_state').val()+' does not exist. Please Create State account.');
        window.location.href = BASE_URL + "index.php?r=gointerdepot%2Findex";
    }
    if($('#totalamount_acc_id_0').val()==""){
        alert('State Account code for '+$('#to_state').val()+' does not exist. Please Create State account.');
        window.location.href = BASE_URL + "index.php?r=gointerdepot%2Findex";
    }
...
function getTotal(){
    ...
    $("#sales_stock_transfer").val(format_money(taxable_amount,2));
    $("#purchase_stock_transfer").val(format_money(taxable_amount,2));
    ...
    for(var i=0; i<invoices; i++){
        ...
        $("#edited_sales_stock_transfer_"+i).val(format_money(taxable_amount,2));
        $("#edited_purchase_stock_transfer_"+i).val(format_money(taxable_amount,2));
        ...
        $("#diff_sales_stock_transfer_"+i).val(format_money(taxable_amount,2));
        $("#diff_purchase_stock_transfer_"+i).val(format_money(taxable_amount,2));
...
function getTotal2(){
    // var taxable_amount = get_number($("#sales_taxable_amount").val(),2);
    // var total_tax = get_number($("#sales_total_tax").val(),2);
    // console.log('taxes'+taxes);

    var taxes = $('.sales_edited-cost').length;
    ...
    var total_igst = 0;
    var total_tax = 0;
    var other_charges = get_number($("#sales_other_charges").val(),2);

    for(var i=0; i<taxes; i++){
        taxable_amount = taxable_amount + get_number($("#sales_total_cost_"+i).val(),2);
        // total_tax = total_tax + get_number($("#sales_total_tax_"+i).val(),2);
        total_cgst = total_cgst + get_number($("#sales_total_cgst_"+i).val(),2);
        total_sgst = total_sgst + get_number($("#sales_total_sgst_"+i).val(),2);
        total_igst = total_igst + get_number($("#sales_total_igst_"+i).val(),2);
        total_tax = total_cgst + total_sgst + total_igst;
    }

    var total_amount = taxable_amount + total_tax + other_charges;
    $("#sales_total_amount").val(format_money(total_amount,2));

    var total_deduction = 0;

    // var shortage_amount = get_number($("#sales_shortage_amount").val(),2);
    // var expiry_amount = get_number($("#sales_expiry_amount").val(),2);
    // var damaged_amount = get_number($("#sales_damaged_amount").val(),2);
    // var margindiff_amount = get_number($("#sales_margindiff_amount").val(),2);
    // var total_deduction = shortage_amount + expiry_amount + damaged_amount + margindiff_amount;
    // $("#sales_total_deduction").val(format_money(total_deduction,2));

    var total_payable_amount = total_amount - total_deduction;
    $("#sales_total_payable_amount").html(format_money(total_payable_amount,2));

    var invoices = $("#no_of_invoices").val();
    // console.log(invoices);

    for(var i=0; i<invoices; i++){
        // taxable_amount = get_number($("#sales_invoice_taxable_amount_"+i).val(),2);
        // total_tax = get_number($("#sales_invoice_total_tax_"+i).val(),2);
        taxable_amount = 0;
        total_cgst = 0;
        total_sgst = 0;
        total_igst = 0;
        total_tax = 0;
        for(var j=0; j<taxes; j++){
            taxable_amount = taxable_amount + get_number($("#sales_invoice_"+i+"_cost_"+j).val(),2);
            // total_tax = total_tax + get_number($("#sales_invoice_"+i+"_tax_"+j).val(),2);
            total_cgst = total_cgst + get_number($("#sales_invoice_"+i+"_cgst_"+j).val(),2);
            total_sgst = total_sgst + get_number($("#sales_invoice_"+i+"_sgst_"+j).val(),2);
            total_igst = total_igst + get_number($("#sales_invoice_"+i+"_igst_"+j).val(),2);
            total_tax = total_cgst + total_sgst + total_igst;
        }
        other_charges = get_number($("#sales_invoice_other_charges_"+i).val(),2);
        total_amount = taxable_amount + total_tax + other_charges;
        $("#sales_invoice_total_amount_"+i).val(format_money(total_amount,2));

        total_deduction = 0;

        // shortage_amount = get_number($("#sales_invoice_shortage_amount_"+i).val(),2);
        // expiry_amount = get_number($("#sales_invoice_expiry_amount_"+i).val(),2);
        // damaged_amount = get_number($("#sales_invoice_damaged_amount_"+i).val(),2);
        // margindiff_amount = get_number($("#sales_invoice_margindiff_amount_"+i).val(),2);
        // total_deduction = shortage_amount + expiry_amount + damaged_amount + margindiff_amount;
        // $("#sales_invoice_total_deduction_"+i).val(format_money(total_deduction,2));

        total_payable_amount = total_amount - total_deduction;
        $("#sales_invoice_total_payable_amount_"+i).html(format_money(total_payable_amount,2));

        // taxable_amount = get_number($("#sales_edited_taxable_amount_"+i).val(),2);
        // total_tax = get_number($("#sales_edited_total_tax_"+i).val(),2);
        taxable_amount = 0;
        total_cgst = 0;
        total_sgst = 0;
        total_igst = 0;
        total_tax = 0;
        for(var j=0; j<taxes; j++){
            taxable_amount = taxable_amount + get_number($("#sales_edited_"+i+"_cost_"+j).val(),2);
            // total_tax = total_tax + get_number($("#sales_edited_"+i+"_tax_"+j).val(),2);
            total_cgst = total_cgst + get_number($("#sales_edited_"+i+"_cgst_"+j).val(),2);
            total_sgst = total_sgst + get_number($("#sales_edited_"+i+"_sgst_"+j).val(),2);
            total_igst = total_igst + get_number($("#sales_edited_"+i+"_igst_"+j).val(),2);
            total_tax = total_cgst + total_sgst + total_igst;
        }
        other_charges = get_number($("#sales_edited_other_charges_"+i).val(),2);
        total_amount = taxable_amount + total_tax + other_charges;
        $("#sales_edited_total_amount_"+i).val(format_money(total_amount,2));

        total_deduction = 0;

        // shortage_amount = get_number($("#sales_edited_shortage_amount_"+i).val(),2);
        // expiry_amount = get_number($("#sales_edited_expiry_amount_"+i).val(),2);
        // damaged_amount = get_number($("#sales_edited_damaged_amount_"+i).val(),2);
        // margindiff_amount = get_number($("#sales_edited_margindiff_amount_"+i).val(),2);
        // total_deduction = shortage_amount + expiry_amount + damaged_amount + margindiff_amount;
        // $("#sales_edited_total_deduction_"+i).val(format_money(total_deduction,2));

        total_payable_amount = total_amount - total_deduction;
        $("#sales_edited_total_payable_amount_"+i).val(format_money(total_payable_amount,2));

        // taxable_amount = get_number($("#sales_diff_taxable_amount_"+i).val(),2);
        // total_tax = get_number($("#sales_diff_total_tax_"+i).val(),2);
        taxable_amount = 0;
        total_cgst = 0;
        total_sgst = 0;
        total_igst = 0;
        total_tax = 0;
        for(var j=0; j<taxes; j++){
            taxable_amount = taxable_amount + get_number($("#sales_diff_"+i+"_cost_"+j).val(),2);
            // total_tax = total_tax + get_number($("#sales_diff_"+i+"_tax_"+j).val(),2);
            total_cgst = total_cgst + get_number($("#sales_diff_"+i+"_cgst_"+j).val(),2);
            total_sgst = total_sgst + get_number($("#sales_diff_"+i+"_sgst_"+j).val(),2);
            total_igst = total_igst + get_number($("#sales_diff_"+i+"_igst_"+j).val(),2);
            total_tax = total_cgst + total_sgst + total_igst;
        }
        other_charges = get_number($("#sales_diff_other_charges_"+i).val(),2);
        total_amount = taxable_amount + total_tax + other_charges;
        $("#sales_diff_total_amount_"+i).val(format_money(total_amount,2));

        total_deduction = 0;

        // shortage_amount = get_number($("#sales_diff_shortage_amount_"+i).val(),2);
        // expiry_amount = get_number($("#sales_diff_expiry_amount_"+i).val(),2);
        // damaged_amount = get_number($("#sales_diff_damaged_amount_"+i).val(),2);
        // margindiff_amount = get_number($("#sales_diff_margindiff_amount_"+i).val(),2);
        // total_deduction = shortage_amount + expiry_amount + damaged_amount + margindiff_amount;
        // $("#sales_diff_total_deduction_"+i).val(format_money(total_deduction,2));

        total_payable_amount = total_amount - total_deduction;
        $("#sales_diff_total_payable_amount_"+i).html(format_money(total_payable_amount,2));
    }
}

$("#get_gointerdepot_qty").click(function(){
    $("#gointerdepot_modal").modal('show');
});

$("#close_gointerdepot_modal").click(function(){
    if($("#form_go_inter_depot_details").valid()){
        check_go_inter_depot_details();
    }
...
$("#get_ledger").click(function(){
    ...
    if (!$("#form_go_inter_depot_details").valid()) {
        ...
        if (check_go_inter_depot_details()==false) {
            ...
            $.ajax({
                url: BASE_URL+'index.php?r=gointerdepot%2Fgetledger',
                type: 'post',
                data: $("#form_go_inter_depot_details").serialize(),
                dataType: 'json',
...
function set_sku_details2(elem){
    ...
    if(elem_id.indexOf("_")>0){
        ...
        if(parseFloat(oldcost)!=parseFloat(newcost)) {
            $('.sales_edited-cost').each(function() {
                var elem_id = $(this).attr('id');
                var index = elem_id.substr(elem_id.lastIndexOf("_")+1);
                var editedcost =  get_number($(this).val());
                var edited_vat_percent = $("#sales_vat_percen_"+index).val();
                if(vat_percen==edited_vat_percent) {
                    if(parseFloat(org_oldcost)==parseFloat(newcost)) {
                       editedcost = editedcost-(sku_qty*org_oldcost);                  
                    } else {
                        editedcost = editedcost-(sku_qty*org_oldcost);
        ...
        if(flag==true) {
            ...
            $.ajax({
                url: BASE_URL+'index.php?r=gointerdepot%2Fgetgoodoutwards',
                type:'post',
                async: false,
                data:{
                    location : $("#to_state").val(),
                    cgst_rate:cgst_rate,
                    sgst_rate:sgst_rate,
                    igs_rate:igst_rate,
                    sku_sgst : sku_sgst,
            ...
            //     //alert(taxes);
            //     $(".select2").select2();
            //     // console.log('edited_0_cost_'+j_count);
            //     getDifference(document.getElementById('edited_0_cost_'+j_count));            
            // },1000);

            $.ajax({
                url: BASE_URL+'index.php?r=gointerdepot%2Fgetgoodoutwards2',
                type:'post',
                async: false,
                data:{
                    location : $("#from_state").val(),
                    cgst_rate:cgst_rate,
                    sgst_rate:sgst_rate,
                    igs_rate:igst_rate,
                    sku_sgst : sku_sgst,
                    ...
                    total :sku_total,
                    vat_percen:vat_percen,
                    j_count:sales_j_count,
                    _csrf : csrfToken
                },
                dataType : 'html',
                success: function (data) {
                    $("#sales_othercharges").before(data);
                }
            });

            // $(this).val('');
            //console.log('sales_j_count'+sales_j_count);

            taxes = $('.sales_edited-cost').length;
            $(".select2").select2();
            getDifference(document.getElementById('sales_edited_0_cost_'+sales_j_count));
...
function get_acc_details(elem){
    ...
    if(elem_id.indexOf("_")>0){
        ...
        $.ajax({
            url: BASE_URL+'index.php?r=gointerdepot%2Fgetaccdetails',
            type: 'post',
            data: {
                    acc_id : acc_id,
                    _csrf : csrfToken
